...
class SPNEGOClientNative:
	...
	def _deep_copy_context(self, ctx):
		...
		ADscan fix: the upstream branch that handles ``ctx.ccred.private_key``
		was missing the final ``return new_ctx`` and silently returned
		``None``, breaking any caller that later expected
		``get_original_context`` to yield a usable Kerberos sub-context
		(e.g. ``DCERPCAuth.from_smb_gssapi`` setting ``auth.kerberos``).
		Without the return, DCE/RPC binds over a Kerberos-only SMB session
		fell back to ``RPC_C_AUTHN_WINNT`` and then dereferenced
		``self.gssapi.ntlm`` (also ``None``) raising
		``'NoneType' object has no attribute 'authenticate'``.
		See ``services/exploitation/raise_child_native.py`` stage 6
		(DRSUAPI bind against the parent DC using a ccache-derived TGS).
		"""
