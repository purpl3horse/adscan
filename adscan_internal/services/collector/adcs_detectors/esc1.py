"""ESC1 — Enrollee Supplies Subject + ClientAuth EKU + low-priv enroll."""
...
def detect_esc1(
    *,
    ...
    """Return ADCSESC1 edges for a vulnerable template, else empty list.

    Conditions:
      * Template kind is CertTemplate
      * msPKI-Certificate-Name-Flag has ENROLLEE_SUPPLIES_SUBJECT bit
      * msPKI-Enrollment-Flag does NOT have PEND_ALL_REQUESTS (no manager approval)
      * msPKI-RA-Signature == 0 (no RA co-sign)
      * Template's EKU list grants client auth (or is empty / any purpose)
    ...
    return [
        make_adcs_edge(principal_sid=sid, template_node=template_node, esc="1")
