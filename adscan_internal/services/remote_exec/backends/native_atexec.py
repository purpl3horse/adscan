"""atexec backend (Task Scheduler / TSCH) — native via aiosmb.

Wraps :meth:`aiosmb.commons.interfaces.machine.SMBMachine.tasks_cmd_exec`.
"""
...
async def execute(config: SMBConfig, command: str, *, timeout: int) -> RemoteExecResult:
    """Run ``command`` via the remote Task Scheduler (atexec-style).
    ...
    Returns:
        :class:`RemoteExecResult` with ``method=ExecMethod.ATEXEC`` and
    ...
    def _open(machine):
        return machine.tasks_cmd_exec(command, result_wait_timeout=timeout)

    return await run_streaming(config, ExecMethod.ATEXEC, _open, timeout=timeout)
